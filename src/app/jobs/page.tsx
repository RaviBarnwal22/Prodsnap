                        ...
                        <aside className="lg:w-72 shrink-0 space-y-10">
                            ...
                            <div>
                                <h3 className="text-sm font-black uppercase tracking-widest text-gray-400 mb-4 flex items-center gap-2">
                                    <MapPin size={14} /> Top Hubs
                                </h3>
                                <div className="flex flex-col gap-2">
                                    <Link href="/jobs" className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${currentRegion === 'ALL' ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}>All Locations</Link>
                                    {['Bangalore', 'Hyderabad', 'Pune', 'Mumbai', 'Delhi NCR', 'Remote'].map((city) => (
                                        <Link
                                            key={city}
                                            href={`/jobs?region=${city}&cat=${currentCat}&type=${currentType}`}
                                            className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${currentRegion === city ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                                        >
                                            {city}
                                        </Link>
                                    ))}
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    <Link href={`/jobs?region=${currentRegion}`} className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${currentCat === 'ALL' ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}>All Roles</Link>
                                    {categories.map(cat => (
                                        <Link
                                            key={cat}
                                            href={`/jobs?cat=${cat}&region=${currentRegion}&type=${currentType}`}
                                            className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${currentCat === cat ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                                        >
                                            {cat}
                                        </Link>
                                    ))}
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    {types.map(type => (
                                        <Link
                                            key={type}
                                            href={`/jobs?type=${type}&region=${currentRegion}&cat=${currentCat}`}
                                            className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${currentType === type ? 'bg-violet-100 dark:bg-violet-900/30 text-violet-600' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                                        >
                            ...
                            <div className="space-y-6">
                                {filteredJobs.length > 0 ? (
                                    filteredJobs.map((job: any) => (
                                        <div
                                            key={job.id}
                                            className="group bg-white dark:bg-white/[0.02] border border-gray-100 dark:border-white/10 rounded-[2rem] p-8 transition-all hover:border-violet-500/30 hover:shadow-2xl hover:shadow-violet-500/5 flex flex-col md:flex-row md:items-center gap-8 relative overflow-hidden"
                                        >
                                            ...
                                            <div className="w-16 h-16 shrink-0 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100 dark:from-white/5 dark:to-white/10 flex items-center justify-center border border-gray-200/50 dark:border-white/10 shadow-inner group-hover:scale-110 transition-transform">
                                                <Building2 className="text-gray-400 group-hover:text-violet-500 transition-colors" size={28} />
                                            </div>
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="mt-6 flex flex-wrap gap-2">
                                                    <span className="px-3 py-1 rounded-full bg-gray-50 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400">
                                                        {job.category}
                                                    </span>
                                                    <span className="px-3 py-1 rounded-full bg-gray-50 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400 flex items-center gap-1.5">
                                            ...
                                            <div className="flex flex-col items-end justify-between self-stretch">
                                                <div className="text-xs font-bold text-gray-300 dark:text-gray-600 flex items-center gap-1.5 pt-1">
                                                ...
                                                <a
                                                    ...
                                                    className="inline-flex items-center gap-2 px-8 py-3.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-black text-sm transition-all hover:scale-105 active:scale-95 group/btn shadow-xl shadow-gray-950/10"
                                                >
                                                    View Details
